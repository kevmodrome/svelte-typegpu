...
export async function loadWorldAssets(signal: AbortSignal, onprogress: (loaded: number) => void): Promise<WorldAssets> {
  ...
  const entries = await Promise.all(Object.entries(assetFiles).map(async ([name, file]) => {
    ...
    if (!asset.meshes.length) throw new Error(`No supported geometry in ${file}`);
    onprogress(++loaded);
...
export const canoePosition: Vector3Tuple = [3.5, 0.025, 3];
export const selectable = [...landmarks.map(({ key, label }) => ({ key, label })), { key: 'canoe', label: 'Canoe' }];
export function selectedPosition(key: string): Vector3Tuple | undefined {
