...
export function createGravityBodies(preset) {
    return presets[preset].map(copyBody);
}
export function stepGravity(bodies, deltaSeconds, speed) {
...
function body(id, position, velocity, mass, radius, color) {
    return { id, position, velocity, mass, radius, color };
}
function asteroidRing(idPrefix, count, minRadius, maxRadius, centerMass, color, direction) {
    return Array.from({ length: count }, (_, index) => {
        ...
        return body(`${idPrefix}-${index}`, [x, ((index % 5) - 2) * 0.05, z], [-Math.sin(angle) * speed * direction, 0, Math.cos(angle) * speed * direction], 0.08, 0.055, color);
    });
}
function dustCloud(idPrefix, count, radius, color, mass) {
    return Array.from({ length: count }, (_, index) => {
        ...
        return body(`${idPrefix}-${index}`, [x, y * radius * 0.45, z], [z * 0.035, -x * 0.01, -x * 0.035], mass, 0.052, color);
    });
}
function copyBody(source) {
    return {
        ...source,
