...
export const worldCounts = [campsiteModelCount, 1000, 5000, 20000, 50000];
export const cellSize = 4.4;
export const compactLandscape = { placements: [], halfWidth: 10, halfDepth: 8 };
function hash(x, z) {
    let value = Math.imul(x, 374761393) ^ Math.imul(z, 668265263);
    value = Math.imul(value ^ (value >>> 13), 1274126177);
    return (value ^ (value >>> 16)) >>> 0;
}
export function createLandscape(count) {
    if (!Number.isInteger(count) || count < campsiteModelCount || count > 50000)
        throw new RangeError('World count must be between 29 and 50000');
    if (count === campsiteModelCount)
        return compactLandscape;
    const placements = [];
    const required = count - campsiteModelCount;
    let radius = 0;
    function place(cx, cz) {
        ...
        if ((Math.abs(x) < 16 && Math.abs(z) < 13) || (x > 0.4 && x < 6.6))
            return;
        // Open trails between groves, independent of requested count.
        if (Math.abs(cx) % 16 === 15 || Math.abs(cz) % 16 === 15)
            return;
