...
function writeGeneratedSourceFile(
  ...
  if (sourceFile.label.endsWith('.svelte')) {
    ...
    if (!custom) {
      // The app compiler owns DOM client/SSR output; importing client-only DOM
      // JavaScript during server builds would eagerly initialize window APIs.
      writeFileSync(outPath, `<!-- Generated by scripts/compile-typegpu-scenes.ts. Do not edit. -->\n${rewriteGeneratedImports(sourceFile.source, sourceFiles, sourceFile.path)}`);
      return;
    }
    const compiled = compileTypeGpu(sourceFile.source, {
      filename: sourceFile.path,
      generate: 'client',
      runes: true,
      experimental: { customRenderer: rendererPath }
    });
...
function rewriteGeneratedImports(
  ...
  for (const sourceFile of sourceFiles) {
    const relative = path.relative(path.dirname(importer), sourceFile.path).replaceAll(path.sep, '/');
    const specifier = relative.startsWith('.') ? relative : `./${relative}`;
    const generatedName = generatedModuleName(sourceFile.path);
    const importSpecifiers =
      path.extname(sourceFile.path) === '.ts'
...
function generatedModuleName(sourcePath: string): string {
  return path
    .basename(sourcePath)
    .replace(/\.typegpu\.svelte$/, '.typegpu.js')
    .replace(/\.ts$/, '.js');
}
